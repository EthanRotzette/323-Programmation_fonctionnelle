...
//linesA.ToList().ForEach(l => Console.WriteLine(l));
...
var diffLines = comparisons.Where(content => content.ContentA != content.ContentB);
//diffLines.ToList().ForEach(n => Console.WriteLine(n.ContentA));

// TODO: 08 Afficher le nombre de lignes identiques et différentes entre les 2 fichiers

Console.WriteLine("Lignes identiques: " + comparisons.Where(content => content.ContentA == content.ContentB).Count());
...
diffLines.ToList().ForEach(diff => 
    Console.WriteLine($"Ligne {diff.NumberHuman} : {
         countVariations(diff)} différences"));
...
Console.WriteLine("\n");
/*
diffLines.ToList().ForEach(diff => Console.WriteLine($">Ligne {diff.NumberHuman} :\n {
    string.Join(',', diff.ContentA.Zip(diff.ContentB, (a,b) => new { Diff = a != b, A = a, B = b, Total=diff })
    .Where(b => b.Diff == true)
    .Select((a) => $"[{a.A}/{a.B}]"))
    }"));
*/
...
diff.ContentA.Zip(diff.ContentB, (a, b) => new { Diff = a != b, A = a, B = b, Total = diff,Text=a!=b?$"[{a}/{b}]" :$"{a}" })
    //.Where(b => b.Diff == true)
    .Select((a) => $"{a.Text}"))}"));

//Pour la couleur: voir ForEach as lieu de Select....
